...
function toggleTheme() {
  ...
  if (window.leafletMap) {
    State.mapStyle = State.theme;
    const darkUrl = 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png';
    const lightUrl = 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png';
    const url = State.mapStyle === 'dark' ? darkUrl : lightUrl;
    window.leafletMap.eachLayer((layer) => {
      if (layer instanceof L.TileLayer) {
        layer.setUrl(url);
      }
    });
    const btn = $('#map-style-toggle-btn');
    if (btn) {
...
async function initSplash() {
  ...
  if (token && userInfo) {
    ...
    State.auth.email = userInfo.email || 'user@communitypulse.ai';
    State.auth.role = userInfo.role || 'admin';
    // Load from local storage info or decode from JWT
    State.auth.name = userInfo.name || 'User';
    State.auth.initials = State.auth.name.split(' ').map(n => n[0]).join('').toUpperCase();
    
    if (splash) splash.style.display = 'none';
    showApp();
    showToast(`Welcome back, ${State.auth.name}!`, 'success');
  } else {
    if (splash) splash.style.display = 'none';
    showLanding();
  }
...
function renderView(view) {
  switch(view) {
    ...
    case 'data':           renderDataConnections(); loadUploadedDatasets(); break;
...
async function handleLogin(e) {
  ...
  try {
    ...
    State.auth.name = profile.name || 'User';
    State.auth.org = profile.org_name || 'CommunityPulse Org';
    State.auth.role = profile.role || 'admin';
    State.auth.initials = State.auth.name.split(' ').map(n => n[0]).join('').toUpperCase();
    
    updateUserUI();
    completeAuthNavigation();
  } catch (err) {
    console.warn("Backend Login failed, trying Firebase/Mock fallback:", err);
    if (window.firebaseAuth && email && password) {
      try {
        ...
        showToast(fbErr.message || 'Login failed. Verify credentials.', 'error');
        if (btn) {
      ...
      // Mock login fallback
      showToast('Login successful (Mock Mode)', 'info');
      State.auth.loggedIn = true;
      State.auth.email = email || 'rajranjeet7680@gmail.com';
      State.auth.name = 'Ranjeet Kumar';
      State.auth.org = 'CommunityPulse AI';
      State.auth.role = 'Super Admin';
      State.auth.initials = 'RK';
      updateUserUI();
      completeAuthNavigation();
    }
  }
...
async function handleSignup(e) {
  ...
  try {
    // 1. Attempt backend API Register
    ...
    State.auth.loggedIn = true;
    State.auth.email = email;
    State.auth.name = profile.name || fullName;
    State.auth.org = profile.org_name || orgName;
    State.auth.role = profile.role || 'admin';
    State.auth.initials = State.auth.name.split(' ').map(n => n[0]).join('').toUpperCase();
    ...
    console.warn("Backend Register failed, trying Firebase/Mock fallback:", err);
    if (window.firebaseAuth && email && password) {
      try {
        ...
        showToast(fbErr.message || 'Signup failed.', 'error');
        if (btn) {
      ...
      showToast('Account created (Mock Mode)', 'info');
      State.auth.loggedIn = true;
      State.auth.email = email;
      State.auth.name = fullName || 'Ranjeet Kumar';
      State.auth.org = orgName;
...
async function handleOAuth(provider) {
  ...
  if (provider === 'Google' && window.firebaseAuth && window.googleAuthProvider) {
    ...
    // General fallback
    setTimeout(() => completeAuthNavigation(), 1200);
  }
}
...
function goToOnboardingStep(step) {
  ...
  $$('.ob-step').forEach(s => {
    const isCurrent = parseInt(s.getAttribute('data-step')) === step;
  ...
  setTimeout(() => {
    if (!window.leafletMap) {
      ...
      if (mapContainer) {
        ...
        const darkUrl = 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png';
        const lightUrl = 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png';
        const tileUrl = State.mapStyle === 'dark' ? darkUrl : lightUrl;
...
function toggleMapStyle() {
  if (!window.leafletMap) return;
  State.mapStyle = State.mapStyle === 'dark' ? 'light' : 'dark';
...
async function loadUploadedDatasets() {
  ...
  try {
    ...
    const section = $('#uploaded-datasets-section');
    const container = $('#uploaded-datasets-list');
    ...
    list.forEach(ds => {
      const sizeMB = (ds.file_size_bytes / (1024 * 1024)).toFixed(2);
      const card = el('div', 'card dataset-card');
      card.style.cssText = 'padding: 20px; cursor: pointer; transition: all var(--t-fast); display: flex; flex-direction: column; gap: 8px; position:relative;';
      card.onclick = () => selectUploadedDataset(ds.dataset_id, ds.name);
      card.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: start;">
          <span class="material-symbols-rounded" style="color: var(--blue); font-size: 28px;">cloud_done</span>
          <div style="display:flex;gap:6px;align-items:center;">
            <span class="badge badge-blue">${sizeMB} MB</span>
            <button onclick="event.stopPropagation(); deleteUploadedDataset('${ds.dataset_id}', '${ds.name}')" class="btn btn-ghost btn-icon btn-sm" style="color:var(--red);padding:2px;min-width:unset;height:unset;" title="Delete dataset">
              <span class="material-symbols-rounded" style="font-size:16px">delete</span>
            </button>
          </div>
        </div>
        <div style="font-weight: 700; font-size: 14px; margin-top: 4px; color: var(--text-primary); text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${ds.name}</div>
        <div style="font-size: 11px; color: var(--text-secondary); line-height: 1.4;">Uploaded on ${new Date(ds.created_at).toLocaleDateString()}</div>
      `;
      container.appendChild(card);
    });
  } catch (err) {
    console.warn("Failed to load uploaded datasets from backend:", err);
  }
}

async function deleteUploadedDataset(datasetId, name) {
  if (!confirm(`Are you sure you want to delete ${name}?`)) return;
  showToast(`Deleting ${name}...`, 'info');
  try {
    await DatasetAPI.delete(datasetId);
    showToast(`✓ Deleted ${name}`, 'success');
    if (State.activeDataset && State.activeDataset.dataset_id === datasetId) {
...
async function selectUploadedDataset(datasetId, name) {
  showToast(`Loading dataset ${name} preview...`, 'info');
  $$('.dataset-card').forEach(card => {
    const isSelected = card.outerHTML.includes(datasetId);
    card.classList.toggle('selected-dataset', isSelected);
  });
  
  try {
    const previewData = await DatasetAPI.preview(datasetId);
    ...
    showToast(`✓ Loaded ${name} with ${previewData.length} preview rows`, 'success');
    renderDatasetExplorer();
  } catch (err) {
    console.error("Failed to load preview:", err);
    showToast(`Failed to preview dataset: ${err.message}`, 'error');
  }
}
...
async function selectDataset(filename) {
  ...
  $$('.dataset-card').forEach(card => {
    const isSelected = card.getAttribute('onclick') && card.getAttribute('onclick').includes(filename);
    card.classList.toggle('selected-dataset', isSelected);
  });
...
async function handleCSVFileUpload(file) {
  ...
  if (State.auth.loggedIn) {
    showToast(`Uploading ${file.name} to FastAPI backend...`, 'info');
    try {
      const response = await DatasetAPI.upload(file);
      showToast(`✓ Successfully uploaded ${file.name}!`, 'success');
      await loadUploadedDatasets();
      await selectUploadedDataset(response.dataset_id, file.name);
    } catch (err) {
      console.warn("Backend dataset upload failed, falling back to local client parsing:", err);
...
function parseCSVLocally(file) {
  showToast(`Parsing ${file.name} locally...`, 'info');
  ...
  reader.onload = function(e) {
    ...
    if (data) {
      ...
      State.connectedSources.push('excel');
      $$('.connector-card[data-connector="excel"]').forEach(c => c.classList.add('connected'));
      showToast(`✓ Loaded local preview for ${file.name}`, 'success');
      renderDatasetExplorer();
    }
  };
...
async function summarizeActiveDataset() {
  if (!State.activeDataset) return;
  ...
  const datasetId = State.activeDataset.dataset_id;
  const filename = State.activeDataset.filename;

  if (datasetId) {
    showToast('Requesting AI dataset summary from backend...', 'info');
    try {
      const data = await AIAPI.summarizeDataset(datasetId);
      thinking.remove();
      
      let summaryText = `### Dataset Summary: **${filename}**\n\n${data.summary}\n\n**Key Takeaways:**\n`;
      data.key_takeaways.forEach(item => {
        summaryText += `- ${item}\n`;
      });
      
      appendAIMessage(summaryText, false, 0.95, ['FastAPI Summarizer', 'Metadata Analyzer']);
    } catch (err) {
      console.warn("Backend summarize failed, falling back to chat request:", err);
      thinking.remove();
...
function appendAIMessage(text, showChart = false, confidence = null, sources = null) {
  const container = $('#chat-messages');
  if (!container) return;
  ...
  const confidencePercent = confidence ? ` (${Math.round(confidence * 100)}% confidence)` : '';
  const sourcesText = sources && sources.length > 0 ? sources.join(', ') : 'BigQuery, IoT Sensors, Govt. API';
    ...
    <div class="chat-bubble ai">
      ...
      <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
        <span style="font-size:11px;color:var(--text-tertiary);display:flex;align-items:center;gap:4px">
          ...
          Sources: ${sourcesText}${confidencePercent}
        </span>
      </div>
...
async function sendChatMessage() {
  ...
  try {
    // 1. Call backend API Chat
    const data = await AIAPI.chat(text);
    thinking.remove();

    // Determine whether to show chart based on keywords
    const lowerText = text.toLowerCase();
    const showChart = lowerText.includes('traffic') || lowerText.includes('congestion') || lowerText.includes('flood') || lowerText.includes('rain') || lowerText.includes('energy') || lowerText.includes('power');
    
    appendAIMessage(data.response, showChart, data.confidence, data.sources);
  } catch (err) {
    ...
    appendAIMessage(response.text, response.chart, 0.85, ['Mock Database', 'Offline Fallback']);
  }
}
